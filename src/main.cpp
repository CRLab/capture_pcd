...
//        nh.getParam("output_frame_id", output_frame_id);
	...
	downsampledCloud->header.frame_id = frame_id;
	...
	pcl::compute3DCentroid (*cloud_transformed, centroid); centroid.w () = 1.f;
	...
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr partial_in_object_frame (new pcl::PointCloud<pcl::PointXYZRGB>);
        ...
        pcl::PointCloud<pcl::PointXYZRGB>::iterator p;
        int num_pts = 0;
        for (p = cloud_transformed->points.begin(); p < cloud_transformed->points.end(); p++)
        {
